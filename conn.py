...
def get_concatenate_df(results_df, relevant_docs_df, topk):
    try:
        ...
        if not isinstance(results_df, pd.DataFrame) or not isinstance(relevant_docs_df, pd.DataFrame):
            raise ValueError("Both results_df and relevant_docs_df must be pandas DataFrames.")
        ...
        if 'Generated Docs' not in results_df.columns or 'Relevant Docs' not in relevant_docs_df.columns:
            raise ValueError("Both DataFrames must contain the necessary columns: 'Generated Docs' and 'Relevant Docs'.")
        
        # Concatenate results_df with relevant_docs_df side by side based on their index
        concatenated_df = pd.concat([results_df.iloc[:topk], relevant_docs_df.iloc[:topk]], axis=1)
        
        # Ensure the concatenated DataFrame has the necessary columns
        if 'Question' not in concatenated_df.columns or 'Relevant Docs' not in concatenated_df.columns or 'Generated Docs' not in concatenated_df.columns:
            raise ValueError("The concatenated DataFrame must contain the necessary columns: 'Question', 'Relevant Docs', and 'Generated Docs'.")
